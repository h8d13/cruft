...
def format_group(i, g):
    lines = []
    lines.append("#{:>2}  {}-line block  x{}  {} wasted lines  {:.0f}%  {}".format(
        i, g.line_count, len(g.occurrences), g.wasted_lines, g.similarity * 100, g.kind,
    ))
...
def report(results, root, total_files, out_path, warnings=None):
    ...
    score_colour = lambda s: GREEN if s >= 0.99 else YELLOW if s >= 0.85 else RED
    wasted_colour = lambda w: RED if w >= 30 else YELLOW if w >= 10 else GREEN
    ...
    print("  Wasted lines    : {}{}{}".format(wasted_colour(total_wasted), total_wasted, RESET))
    print("  Full report     : {}{}{}".format(GREEN, out_path, RESET))
    print(bar)

    if not results:
        print("  {}No duplicate blocks found -- clean codebase!{}\n".format(GREEN, RESET))
        return

    # print top N to terminal with colour
    for i, g in enumerate(results[:TOP_N], 1):
        print("{}{}#{:>2}  {}-line block  x{}  {} wasted lines  {}{}{:.0f}%{}  {}{}{}".format(
            BOLD, wasted_colour(g.wasted_lines),
            i, g.line_count, len(g.occurrences), g.wasted_lines,
            score_colour(g.similarity), BOLD, g.similarity * 100, RESET,
            DIM, g.kind, RESET,
        ))
        for occ in g.occurrences:
    ...
    if len(results) > TOP_N:
        print("  {}... and {} more -- see {}{}".format(DIM, len(results) - TOP_N, out_path, RESET))

    # WRITE THE FULL TXT
    with open(str(out_path), 'w') as fh:
        fh.write("scan report -- {}\n".format(root))
        fh.write("files: {}  groups: {}  wasted lines: {}\n".format(
