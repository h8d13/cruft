...
# STRUCS
class Occurrence:
    def __init__(self, file, start_line, end_line, lines, norm_lines=None, fuzzy_lines=None):
        self.file        = file
...
class DuplicateGroup:
    ...
    @property
    def wasted_lines(self): return self.line_count * (len(self.occurrences) - 1)
    @property
    def kind(self):         return "exact" if self.similarity >= 0.99 else "fuzzy"

    def preview(self, max_lines=5):
        snippet = self.lines[:max_lines]
        tail = '\n    ... ({} more lines)'.format(self.line_count - max_lines) if self.line_count > max_lines else ''
    ...
    for line in raw_lines:
        ...
        while True:
            ...
            if close_idx == -1:
                ...
                clean = clean[:earliest_pos] + after_open[close_idx + len(earliest_close):]
        result.append(clean)
    return result
...
def similarity(a_lines, b_lines):
    a = ' '.join(l.strip() for l in a_lines)
    b = ' '.join(l.strip() for l in b_lines)
...
def collect_files(root, spec=None):
    ...
    for path in root.rglob('*'):
        ...
        if spec and spec(path):
            continue
        if path.is_file() and (path.suffix.lower() in EXTENSIONS or (not path.suffix and not path.name.startswith('.'))) and not is_binary(path):
            files.append(path)
    return sorted(files)
...
def scan(files):
    ...
    for i, path in enumerate(files, 1):
        ...
        print("\r  [{}] {:5.1f}%  {}{}{}".format(bar, pct, GREEN, label, RESET), end='', flush=True)
        ...
        exact_norm = [norm_exact(l) for l in stripped]
        fuzzy_norm = [norm_fuzzy(l) for l in stripped]
        n = len(raw)
        ...
        for start in range(n - MIN_LINES + 1):
            ...
            if sum(bool(l) for l in e_chunk) < MIN_LINES * 0.6:
                continue

            eh  = hash_lines(e_chunk)
            occ = Occurrence(path, start + 1, start + MIN_LINES, block, e_chunk, f_chunk)
            occ.exact_hash = eh

            # Exact pass
            exact_index.setdefault(eh, DuplicateGroup(block, 1.0)).occurrences.append(occ)

            # Fuzzy pass... only index if not already an exact match
            fh = hash_lines(f_chunk)
            if fh != eh:
                fuzzy_index.setdefault(fh, DuplicateGroup(block, 0.0)).occurrences.append(occ)
    ...
    for g in seeds:
        ...
        while True:
            ...
            next_sig = frozenset((o.file, o.start_line + (g.line_count - MIN_LINES + 1))
    ...
    for g in fuzzy_index.values():
        ...
        scores = [similarity(ref, o.norm_lines if o.norm_lines else o.lines)
                  for o in reps[1:]]
        g.similarity = sum(scores) / len(scores)
        if g.similarity >= FUZZY_THRESHOLD:
            results.append(g)

    return sorted(results, key=lambda g: (g.wasted_lines, g.similarity), reverse=True), warnings
